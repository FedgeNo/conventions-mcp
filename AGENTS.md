...
setup steps. This file is guidance for any coding agent working on the server's
own code, plus client-specific instructions for wiring the finished server into
supported clients.
...
"Claude Code only" under "Wiring this server into agent clients" below),
enforce baseline rule retrieval before tool use — hook events fire at the moment
...
- **`PreToolUse`** (matcher `*`) → `bin/pre-tool-check.js` — fires before
  *every* tool call and actually denies it (`permissionDecision: "deny"`,
  ...
  when set, else `~/.conventions-mcp/hook-state`) containing
